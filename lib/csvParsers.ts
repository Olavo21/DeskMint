...
export const BROKERS: { id: BrokerId; label: string; icon: string; hasApi: boolean; csvHelp: string }[] = [
  ...
  },
  {
    ...
    icon:    '🔵',
    hasApi:  false,
    csvHelp: 'App XTB (móvel): perfil → Histórico de transações → data: Tudo → Download → Excel (.xlsx)\n⚠️ Não uses "Operações em numerário" — esse ficheiro não tem posições.\n\nxStation web (mais preciso): Histórico de conta → Ordens → Export → Excel ou HTML\n\nAceita: .xlsx (Excel) ou .html (xStation)',
  },
  {
    id:      'traderepublic',
